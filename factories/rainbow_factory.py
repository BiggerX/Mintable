...
from factories.lib import random_rainbow
...
def mint():
    ...
    db = {}
    collection_name = u'random_rainbow_series'
    ...
    while(count <= max_count):
        ...
        file_name = "images/random_rainbow_series/random_rainbow_token_" + \
            str(count) + ".png"
        my_file = Path(file_name)
        if not my_file.is_file():
            # * Create the art if file doesn't exist
            random_rainbow.main(
                file_name, "Rainbow Mirror " + str(count), 1200, 1200)
        else:
