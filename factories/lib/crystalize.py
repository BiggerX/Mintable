...
def main(inputFile, outputFile):
    ...
    im = im.resize(size, PIL.Image.LANCZOS).rotate(float(0))
    im = im.convert('RGBA')
    ...
    for i in range(34):
        ...
        topLeftY += 5
        rotation -= 5
        curr = im.resize((int(size[0] - sqrt(topLeftX)),
                          int(size[1] - sqrt(topLeftY))), PIL.Image.LANCZOS)
        curr = curr.rotate(rotation)

        out.paste(curr, rotatePoint(topLeftX, topLeftY,
                                    centerPoint[0], centerPoint[1], rotation), curr)
    # Save the image out
