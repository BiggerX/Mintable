...
from factories.lib import invader
...
def mint():
    ...
    db = {}
    collection_name = u'invader_series'
    ...
    while(count <= 10):
        ...
        collection_title = 'Invader Series Token ' + str(count)
        collection_subtitle = 'Number ' + str(count) + '/10 Tokens'
        collection_description = 'Part ' + \
            str(count) + ' out of 10 in the Invader Dive Series by Toxic Mushroom. Nature\'s patterns present themselves everywhere in our daily lives. This series captures the Pixelation of life.'
        token_price = '30'

        file_name = "images/invader_series/invader_token_" + \
            str(count) + ".png"
        my_file = Path(file_name)
        if not my_file.is_file():
            # * Create the art if file doesn't exist
            invader.main(8, 40, 2000, file_name)
        else:
